...
function login() {
  ...
  var myWindow = window.open(window.location.origin + '/wait', 'twitter login',
    'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=495,width=645'
  );
  fetch('/auth/twitter', {
    method: 'POST',
    mode: 'cors',
    ...
    data = j
    var qs = 'oauth_callback_confirmed=' + data['oauth_callback_confirmed'] +
      '&oauth_token=' + data['oauth_token'] + '&oauth_token_secret=' + data[
        'oauth_token_secret']
